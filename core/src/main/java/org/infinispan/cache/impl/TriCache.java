 ...
 * @since  1.0
 */
public abstract class TriCache<K,V> implements Receiver, Closeable, Runnable, DiagnosticsHandler.ProbeHandler {
    // protected final Map<K,V>                           map=new ConcurrentHashMap<>();
    protected final Log                                logger=LogFactory.getLog(TriCache.class);
    ...
    protected BlockingQueue<Data<K,V>>           put_queue;
    protected final LongAdder                    num_msgs_received=new LongAdder();
    protected Runner                             put_handler;
    protected static final ThreadFactory         thread_factory=new DefaultThreadFactory("tri", false, true)
                                                                    .useVirtualThreads(true);

    protected String cfg_str, name_str;

    static {
        ClassConfigurator.add((short)1500, Data.class);
        ClassConfigurator.add((short)1501, WrappedByteArray.class);
    }


    public TriCache(String config, String name) {
        this.cfg_str=config;
        this.name_str=name;
        this.is_trace=logger.isTraceEnabled();
    ...
    public void start() {
        try {
            ch=new JChannel(cfg_str).setReceiver(this);
            if(name_str != null)
                ch.name(name_str);
            ch.connect("simple");
            ...
            logger.info("I'm %s, backup is %s", local_addr, backup);
    ...
    protected abstract V _put(K key, V value);
    ...
    public V put(K key, V value) {
        ...
        try {
            ...
            if(Objects.equals(primary, local_addr)) {
                if(is_trace)
                    logger.trace("%s: local put (req=%d key=%s)", local_addr, req_id, key);
                put_queue.add(data);
            }
            else {
                if(is_trace)
                    logger.trace("%s: sending put(%s) to primary %s (req=%d)", local_addr, key, primary, req_id);
                send(primary, data);
            }
    ...
    protected abstract V _get(Object key);
    ...
    public V get(Object key) {
        ...
        if(get_is_local) {
            if(is_trace)
                logger.trace("%s: handling get(%s) locally", local_addr, key);
            return _get(key);
        }
    ...
    public void viewAccepted(View view) {
        ...
        if(local_addr != null) {
            ...
            primary_is_backup=Objects.equals(local_addr, backup);
            logger.info("I'm %s, backup is %s (primary %s backup)", local_addr, backup, primary_is_backup? "==" : "!=");
        }
    }
    ...
    protected void handlePut(Data<K,V> data) {
        ...
        if(primary_is_backup) { // primary == backup (e.g. when cluster size is 1: ack directly
            ...
            if(m != null)
                logger.trace("%s, sending backup(%s) to %S", m, data.key, backup);
    ...
    protected void handleBackup(Data<K,V> data, Address sender) {
        ...
        if(is_trace)
            logger.trace("%s: received backup(%s) from %s (req=%d)", local_addr, data.key, sender, data.req_id);
        ...
        if(Objects.equals(local_addr, dest)) {
            if(is_trace)
                logger.trace("%s: handling ack(%s) locally (req=%d)", local_addr, key, data.req_id);
            handleAck(data);
        }
        else {
            if(is_trace)
                logger.trace("%s: sending ack(%s) to original requester %s (req=%d)", local_addr, key, dest, data.req_id);
    ...
    protected void handleAck(Data<K,V> data) {
        ...
        if(future != null) {
            ...
            if(is_trace)
                logger.trace("%s: ack (req=%d)", local_addr, data.req_id);
        }
    }
    ...
    protected void process(Data<K,V> data, Address sender) throws Exception {
        ...
        switch(data.type) {
            ...
            case GET:
                if(is_trace)
                    logger.trace("%s: received get(%s) from %s (req=%d), sending response", local_addr, data.key, sender, data.req_id);
    ...
    protected void sendData(Address dest, Data<K,V> data, boolean oob) {
        try {
            ...
            logger.error("%s: failed sending data to %s: %s", local_addr, data.sender, t);
        }
    }
